...
ArchNetworkWinsock::initModule (HMODULE module) {
    ...
    setfunc (recv_winsock,
             recv,
             int(PASCAL FAR*) (SOCKET s, void FAR* buf, int len, int flags));
    setfunc (select_winsock,
             select,
             int(PASCAL FAR*) (int nfds,
                               fd_set FAR* readfds,
                               fd_set FAR* writefds,
                               fd_set FAR* exceptfds,
                               const struct timeval FAR* timeout));
...
ArchNetworkWinsock::bindSocket (ArchSocket s, ArchNetAddress addr) {
    ...
    if (bind_winsock (s->m_socket, &addr->m_addr, addr->m_len) ==
        SOCKET_ERROR) {
        throwError (getsockerror_winsock ());
    }
}
...
ArchNetworkWinsock::connectSocket (ArchSocket s, ArchNetAddress addr) {
    ...
    if (connect_winsock (s->m_socket, &addr->m_addr, addr->m_len) ==
        SOCKET_ERROR) {
...
ArchNetworkWinsock::pollSocket (PollEntry pe[], int num, double timeout) {
    ...
    // wait
    DWORD result =
        WSAWaitForMultipleEvents_winsock (n, events, FALSE, t, FALSE);
...
ArchNetworkWinsock::setNoDelayOnSocket (ArchSocket s, bool noDelay) {
    ...
    size      = sizeof (flag);
    if (setsockopt_winsock (
            s->m_socket, IPPROTO_TCP, TCP_NODELAY, &flag, size) ==
        SOCKET_ERROR) {
        throwError (getsockerror_winsock ());
    }
...
ArchNetworkWinsock::setReuseAddrOnSocket (ArchSocket s, bool reuse) {
    ...
    size      = sizeof (flag);
    if (setsockopt_winsock (
            s->m_socket, SOL_SOCKET, SO_REUSEADDR, &flag, size) ==
        SOCKET_ERROR) {
        throwError (getsockerror_winsock ());
    }
