...
MSWindowsScreen::MSWindowsScreen (bool isPrimary, bool noHooks,
                                  bool stopOnDeskSwitch, IEventQueue* events)
    ...
    try {
        ...
        m_desks       = new MSWindowsDesks (m_isPrimary,
                                      m_noHooks,
                                      m_hook.getInstance (),
                                      m_screensaver,
                                      m_events,
                                      ...
                                      stopOnDeskSwitch);
        m_keyState =
            new MSWindowsKeyState (m_desks, getEventTarget (), m_events);
        ...
        if (SUCCEEDED (
            ...
            LOG ((CLOG_DEBUG "using desktop for drop target: %s",
                  m_desktopPath.c_str ()));
        } else {
            LOG ((CLOG_ERR "failed to get desktop path, no drop target "
                           "available, error=%d",
                  GetLastError ()));
        }
    ...
    m_events->adoptHandler (Event::kSystem,
                            ...
                            new TMethodEventJob<MSWindowsScreen> (
                                this, &MSWindowsScreen::handleSystemEvent));
...
MSWindowsScreen::enable () {
    ...
    m_events->adoptHandler (Event::kTimer,
                            m_fixTimer,
                            new TMethodEventJob<MSWindowsScreen> (
                                this, &MSWindowsScreen::handleFixes));
...
MSWindowsScreen::leave () {
    ...
    if (m_isPrimary) {
        ...
        // warp to center
        LOG ((CLOG_DEBUG1 "warping cursor to center: %+d, %+d",
              m_xCenter,
              m_yCenter));
        ...
        for (KeyButton i = 0; i < IKeyState::kNumButtons; ++i) {
            if (m_keyState->isKeyDown (i)) {
                ...
                LOG ((CLOG_DEBUG1
                      "key button %d is down before leaving to another screen",
                      i));
            }
        }
    }
...
MSWindowsScreen::sendDragThread (void*) {
    ...
    if (draggingFilename.empty () == false) {
        ...
        UInt32 fileCount = 1;
        LOG ((CLOG_DEBUG "send dragging info to server: %s",
              draggingFilename.c_str ()));
...
MSWindowsScreen::checkClipboards () {
    ...
    if (m_ownClipboard && !MSWindowsClipboard::isOwnedBySynergy ()) {
        ...
        m_ownClipboard = false;
        sendClipboardEvent (m_events->forClipboard ().clipboardGrabbed (),
                            kClipboardClipboard);
        sendClipboardEvent (m_events->forClipboard ().clipboardGrabbed (),
                            kClipboardSelection);
    }
...
MSWindowsScreen::warpCursor (SInt32 x, SInt32 y) {
    ...
    while (PeekMessage (&msg,
                        NULL,
                        SYNERGY_MSG_INPUT_FIRST,
                        SYNERGY_MSG_INPUT_LAST,
                        PM_REMOVE)) {
        // do nothing
    }
...
void
MSWindowsScreen::saveMousePosition (SInt32 x, SInt32 y) {
    m_xCursor = x;
    m_yCursor = y;
...
MSWindowsScreen::isAnyMouseButtonDown (UInt32& buttonID) const {
    static const char* buttonToName[] = {"<invalid>",
                                         "Left Button",
                                         "Middle Button",
                                         "Right Button",
                                         "X Button 1",
...
bool
MSWindowsScreen::fakeKeyRepeat (KeyID id, KeyModifierMask mask, SInt32 count,
                                KeyButton button) {
...
MSWindowsScreen::createBlankCursor () const {
    ...
    HCURSOR c =
        CreateCursor (s_windowInstance, 0, 0, cw, ch, cursorAND, cursorXOR);
    delete[] cursorXOR;
    delete[] cursorAND;
    return c;
}
...
MSWindowsScreen::createWindow (ATOM windowClass, const char* name) const {
    HWND window =
        CreateWindowEx (WS_EX_TOPMOST | WS_EX_TRANSPARENT | WS_EX_TOOLWINDOW,
...
MSWindowsScreen::createDropWindow (ATOM windowClass, const char* name) const {
    HWND window =
        CreateWindowEx (WS_EX_TOPMOST | WS_EX_TRANSPARENT | WS_EX_ACCEPTFILES,
...
MSWindowsScreen::updateButtons () {
    ...
    m_buttons[kButtonExtra0 + 0] =
        (numButtons >= 4) && (GetKeyState (VK_XBUTTON1) < 0);
    m_buttons[kButtonExtra0 + 1] =
        (numButtons >= 5) && (GetKeyState (VK_XBUTTON2) < 0);
}
...
bool
MSWindowsScreen::onPreDispatch (HWND hwnd, UINT message, WPARAM wParam,
                                LPARAM lParam) {
    // handle event
    switch (message) {
...
bool
MSWindowsScreen::onPreDispatchPrimary (HWND, UINT message, WPARAM wParam,
                                       LPARAM lParam) {
    ...
    switch (message) {
        ...
        case SYNERGY_MSG_PRE_WARP: {
            // save position to compute delta of next motion
            saveMousePosition (static_cast<SInt32> (wParam),
                               static_cast<SInt32> (lParam));
            ...
            do {
                GetMessage (
                    &msg, NULL, SYNERGY_MSG_MOUSE_MOVE, SYNERGY_MSG_POST_WARP);
...
bool
MSWindowsScreen::onEvent (HWND, UINT msg, WPARAM wParam, LPARAM lParam,
                          LRESULT* result) {
    switch (msg) {
        ...
        case WM_CHANGECBCHAIN:
            if (m_nextClipboardWindow == (HWND) wParam) {
                ...
                LOG ((CLOG_DEBUG "clipboard chain: new next: 0x%08x",
        ...
        case WM_POWERBROADCAST:
            switch (wParam) {
                ...
                case PBT_APMRESUMESUSPEND:
                    m_events->addEvent (
                        Event (m_events->forIScreen ().resume (),
                               getEventTarget (),
                               NULL,
                ...
                case PBT_APMSUSPEND:
                    m_events->addEvent (
                        Event (m_events->forIScreen ().suspend (),
                               getEventTarget (),
                               NULL,
                               Event::kDeliverImmediately));
                    break;
...
MSWindowsScreen::onKey (WPARAM wParam, LPARAM lParam) {
    ...
    LOG ((CLOG_DEBUG1 "event: Key char=%d, vk=0x%02x, nagr=%d, lParam=0x%08x",
          (wParam & 0xff00u) >> 8,
          wParam & 0xffu,
          (wParam & 0x10000u) ? 1 : 0,
          lParam));
    ...
    if (m_keyState->testAutoRepeat (
            down, (lParam & 0x40000000u) == 1, button)) {
        lParam |= 0x40000000u;
    }
    ...
    if (!down && m_isPrimary && !m_isOnScreen) {
        PrimaryKeyDownList::iterator find = std::find (
            m_primaryKeyDownList.begin (), m_primaryKeyDownList.end (), button);
...
MSWindowsScreen::onMouseMove (SInt32 mx, SInt32 my) {
    ...
    if (m_isOnScreen) {
        ...
        // secondary screen.
        LOG ((CLOG_DEBUG5 "warping server cursor to center: %+d,%+d",
              m_xCenter,
              m_yCenter));
...
MSWindowsScreen::warpCursorNoFlush (SInt32 x, SInt32 y) {
    ...
    if (!isPrimary ()) {
        if ((cursorPos.x != x) && (cursorPos.y != y)) {
            LOG ((CLOG_DEBUG
                  "SetCursorPos did not work; using fakeMouseMove instead"));
            LOG ((CLOG_DEBUG "cursor pos %d, %d expected pos %d, %d",
                  cursorPos.x,
                  cursorPos.y,
                  x,
                  y));
            // when at Vista/7 login screen, SetCursorPos does not work (which
            // could be
            // an MS security feature). instead we can use fakeMouseMove, which
            // calls
            // mouse_event.
            // IMPORTANT: as of implementing this function, it has an annoying
            // side
            // effect; instead of the mouse returning to the correct exit point,
            // it
            // returns to the center of the screen. this could have something to
            // do with
            // the center screen warping technique used (see comments for
            // onMouseMove
            // definition).
            fakeMouseMove (x, y);
        }
        ...
        for (KeyButton i = 0; i < IKeyState::kNumButtons; ++i) {
            if (down[i] && !m_keyState->isKeyDown (i)) {
                m_keyState->sendKeyEvent (
                    getEventTarget (), false, false, kKeyNone, mask, 1, i);
            }
        }
    }
}
...
MSWindowsScreen::forceShowCursor () {
    ...
    if (showMouse != m_showingMouse) {
        if (showMouse) {
            ...
            m_gotOldMouseKeys     = (SystemParametersInfo (SPI_GETMOUSEKEYS,
    ...
    if (m_draggingStarted) {
        ...
        if (!filename.empty ()) {
            if (DragInformation::isFileValid (filename)) {
                ...
                LOG ((CLOG_DEBUG "drag file name is invalid: %s",
                      filename.c_str ()));
            }
        }
