...
void
MSWindowsDesks::setShape (SInt32 x, SInt32 y, SInt32 width, SInt32 height,
...
void
MSWindowsDesks::fakeKeyEvent (KeyButton button, UINT virtualKey, bool press,
    ...
    sendMessage (SYNERGY_MSG_FAKE_KEY,
                 flags,
...
MSWindowsDesks::queryHookLibrary (HINSTANCE hookLibrary) {
    ...
    if (m_isPrimary && !m_noHooks) {
        ...
        if (m_install == NULL || m_uninstall == NULL ||
            m_installScreensaver == NULL || m_uninstallScreensaver == NULL) {
...
LRESULT CALLBACK
MSWindowsDesks::primaryDeskProc (HWND hwnd, UINT msg, WPARAM wParam,
                                 LPARAM lParam) {
    return DefWindowProc (hwnd, msg, wParam, lParam);
}

LRESULT CALLBACK
MSWindowsDesks::secondaryDeskProc (HWND hwnd, UINT msg, WPARAM wParam,
                                   LPARAM lParam) {
...
MSWindowsDesks::deskLeave (Desk* desk, HKL keyLayout) {
    ...
    if (m_isPrimary) {
        ...
        LOG ((CLOG_DEBUG2 "warping cursor to center: %+d,%+d",
              m_xCenter,
              m_yCenter));
...
MSWindowsDesks::deskThread (void* vdesk) {
    ...
    if (desk->m_desk != NULL && SetThreadDesktop (desk->m_desk) != 0) {
        ...
        try {
            ...
            LOG ((CLOG_DEBUG "desk %s window is 0x%08x",
                  desk->m_name.c_str (),
                  desk->m_window));
        } catch (...) {
            // ignore
            LOG ((CLOG_DEBUG "can't create desk window for %s",
                  desk->m_name.c_str ()));
        }
    }
        ...
        switch (msg.message) {
            ...
            case SYNERGY_MSG_SWITCH:
                if (m_isPrimary && !m_noHooks) {
                    ...
                    if (desk->m_window)
                        EnableWindow (desk->m_window,
                                      desk->m_lowLevel ? FALSE : TRUE);
                }
                break;
            ...
            case SYNERGY_MSG_FAKE_BUTTON:
                if (msg.wParam != 0) {
                    mouse_event (
                        (DWORD) msg.wParam, 0, 0, (DWORD) msg.lParam, 0);
                }
                break;
            ...
            case SYNERGY_MSG_FAKE_WHEEL:
                ...
                if (msg.lParam != 0) {
                    mouse_event (
                        MOUSEEVENTF_WHEEL, 0, 0, (DWORD) msg.lParam, 0);
                }
                break;
...
MSWindowsDesks::removeDesks () {
    for (Desks::iterator index = m_desks.begin (); index != m_desks.end ();
         ++index) {
...
MSWindowsDesks::openInputDesktop () {
    return OpenInputDesktop (DF_ALLOWOTHERACCOUNTHOOK,
                             TRUE,
