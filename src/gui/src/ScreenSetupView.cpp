...
void
ScreenSetupView::setModel (ScreenSetupModel* model) {
    QTableView::setModel (model);
    setTableSize ();
}

ScreenSetupModel*
ScreenSetupView::model () const {
...
void
ScreenSetupView::resizeEvent (QResizeEvent* event) {
    setTableSize ();
    event->ignore ();
}

void
ScreenSetupView::mouseDoubleClickEvent (QMouseEvent* event) {
...
void
ScreenSetupView::dragEnterEvent (QDragEnterEvent* event) {
...
void
ScreenSetupView::dragMoveEvent (QDragMoveEvent* event) {
    if (event->mimeData ()->hasFormat (ScreenSetupModel::mimeType ())) {
        ...
        if (event->source () == this) {
            ...
            // a drop from outside is not allowed if there's a screen already
            // there.
            if (!model ()->screen (col, row).isNull ())
                event->ignore ();
            else
...
void
ScreenSetupView::startDrag (Qt::DropActions) {
...
ScreenSetupView::viewOptions () const {
